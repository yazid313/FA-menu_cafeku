...
export default function Addkategory({ params }) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        if (slug === "edit") {
          const idKategory = localStorage.getItem("id_kategory");

          const response = await axios.get(
            `${process.env.NEXT_PUBLIC_BASE_API_URL}/api/v1/category/show/${idKategory}`
          );
  ...
  const handleSubmit = async (e) => {
    ...
    formData.append("type", kategory.type);

    if (selectedFile) {
      formData.append("photo", selectedFile);
    } else if (kategory.photo) {
      formData.append("photo", kategory.photo);
    }
    ...
    try {
      ...
      if (kategory.id) {
        ...
        await axios.put(
          `${process.env.NEXT_PUBLIC_BASE_API_URL}/api/v1/category/update/${kategory.id}`,
          formData,
          { headers }
        );
        localStorage.removeItem("id_kategory");
  ...
  return (
    <div className="p-8 pt-20 w-full">
      <h2 className="text-xl font-nunito">Manage kategory</h2>
      {isLoading ? (
        <EditDataSkeleton />
      ) : (
              ...
              <img
                src={
                  slug === "create"
                    ...
                    : kategory.photo !== selectedFile
                    ? URL.createObjectURL(selectedFile)
                    : `${process.env.NEXT_PUBLIC_BASE_API_URL}/${kategory.photo}`
                }
                alt="event Preview"
